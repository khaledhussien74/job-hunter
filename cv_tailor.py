...
# ---------------------------------------------------------------------------
# Cover letter (FREE local template by default; AI rewrite if a key is set)
# ---------------------------------------------------------------------------
def _local_cover_letter(job, master, target_title):
    """Build a truthful cover letter from master CV facts — no API needed."""
    name = master.get("name", "").title() if master.get("name", "").isupper() \
        else master.get("name", "")
    company = (job.get("company") or "").strip()
    summary = ROLE_SUMMARIES.get(target_title) or master.get("summary", "")
    who = f"the {target_title} position" + (f" at {company}" if company else "")
    para1 = (f"I am writing to express my strong interest in {who}. " + summary)
    para2 = ("My experience maps directly to what this role needs, and I would "
             "welcome the opportunity to bring it to your team and help you hit "
             "your growth and revenue goals.")
    return f"Dear Hiring Manager,\n\n{para1}\n\n{para2}\n\nBest regards,\n{name}"


def _anthropic_cover_letter(job, master, target_title, api_key, model):
    system = (
        "You are an expert cover-letter writer. Write a concise, professional "
        "cover letter (max ~200 words) for the applicant applying to the job. "
        "Use ONLY facts consistent with the applicant summary — do NOT invent "
        "employers, dates, metrics, or skills. Output ONLY the letter body "
        "(no markdown, no bracketed placeholders); sign off with the "
        "applicant's name.")
    user = {
        "applicant_name": master.get("name", ""),
        "applicant_summary": master.get("summary", ""),
    ...
    body = json.dumps({
        ...
        "messages": [{"role": "user", "content":
                      "Write the cover letter for this:\n\n"
                      + json.dumps(user, ensure_ascii=False)}],
    }).encode("utf-8")
    req = Request(ANTHROPIC_ENDPOINT, data=body, headers={
        "content-type": "application/json",
...
def render_cover_letter_pdf(text, master, target_title, out_path):
    """Render a clean one-page cover-letter PDF. Requires fpdf2."""
    from fpdf import FPDF
    ...
    pdf.set_text_color(70, 70, 70)
    pdf.cell(0, 6, s(target_title), new_x="LMARGIN", new_y="NEXT")
    cline = "  |  ".join(v for v in [contact.get("email"), contact.get("phone"),
    ...
    for para in (text or "").split("\n"):
        if para.strip():
            pdf.multi_cell(0, 6, s(para), new_x="LMARGIN", new_y="NEXT")
        else:
            pdf.ln(3)
    pdf.output(out_path)
    return out_path
...
def build_cover_letter_for_job(job, master, out_dir, api_key=None, model=None):
    """Build a tailored cover-letter PDF. Returns (pdf_path, info_dict) where
    info_dict has a 'title' key. Free local template by default; uses Claude
    only when an ANTHROPIC_API_KEY is available."""
    api_key = (api_key if api_key is not None
               else os.environ.get("ANTHROPIC_API_KEY", "")).strip()
    allowed = master.get("allowed_titles",
                         [master.get("default_title", "Marketing Manager")])
    target_title = heuristic_title(job, allowed)
    text = None
    if api_key:
        try:
            text = _anthropic_cover_letter(job, master, target_title,
                                           api_key, model or DEFAULT_MODEL)
        except Exception as e:
            print("  [x] cover letter (API) failed, using local:", e)
    if not text:
        text = _local_cover_letter(job, master, target_title)
    fname = "Khaled_Hussien_Cover_Letter_" + _slug(target_title) + ".pdf"
    out_path = os.path.join(out_dir, fname)
    render_cover_letter_pdf(text, master, target_title, out_path)
    return out_path, {"title": target_title, "text": text}
