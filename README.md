...
## 📄 CV مفصّل لكل وظيفة (ATS-Friendly) — مجاني بالكامل

مع كل وظيفة جديدة، البوت بيبني **نسخة CV مخصّصة للوظيفة دي** من الـ CV الماستر
(`cv_master.json`) وبيبعتها مرفقة تحت الوظيفة على تيليجرام (ملف PDF).
**كله شغّال محليًا ومجانًا — من غير أي API key ولا أي توكن ولا تسجيل في أي حاجة.**

**بيعمل إيه بالظبط:**
1. **بيختار المسمّى الوظيفي الأنسب** للوظيفة من بين:
   `Marketing Manager` / `Digital Marketing Manager` /
   `Performance Marketing Manager` / `Growth Marketing Manager` —
   حسب عنوان الوظيفة ومتطلباتها (بالكلمات المفتاحية).
2. **بيحط ملخّص (Summary) مخصّص للمسمّى ده** — في ٤ نسخ جاهزة (واحدة لكل مسمّى)
   كلها مكتوبة من بيانات الـ CV الماستر بتاعك.
3. **بيعيد ترتيب الـ Core Competencies** حسب علاقتها بكلمات الوظيفة، فالأهم
   للوظيفة يظهر الأول (ده اللي بيرفع مطابقة الـ ATS لكل إعلان).
4. **بيطلّع PDF متوافق مع أنظمة الـ ATS:** عمود واحد، خط قياسي، عناوين أقسام
   واضحة، **نص قابل للقراءة آليًا**، من غير جداول/صور/أيقونات بتلخبط الـ ATS.

> 🔒 **مفيش أي اختراع بيانات:** كل الخبرات والشركات والتواريخ والأرقام والتعليم
> والمهارات بتتاخد **حرفيًا** من الـ CV الماستر. البوت بيغيّر **بس** المسمّى
> والملخّص وترتيب المهارات. يعني الـ CV بيفضل صادق ١٠٠٪.

> 🛠️ **التعديل على الـ CV الماستر:** غيّر محتواك في `cv_master.json` (الملخّص،
> المهارات، الخبرات … إلخ) وكل الوظايف الجاية هتتفصّل من المحتوى الجديد. لو عايز
> تظبط نص الملخّصات حسب كل مسمّى، عدّل `ROLE_SUMMARIES` في `cv_tailor.py`.
> الـ workflow بيثبّت مكتبة `fpdf2` لتوليد الـ PDF — وهي مكتبة مجانية.

> 💡 عشان توقّف ميزة الـ CV خالص حُط متغير `ATTACH_CV=0`.
